...
function parseXdna(buffer: Buffer) {
  if (buffer.length < HEADER_SIZE) {
    fail(`File is too short for an XDNA header (${buffer.length} bytes)`);
  }
  ...
  return {
    ...
    offsets: {
      ...
      annotations: annotations
        ? { start: commentOffset + commentLength, end: buffer.length }
        : null,
    },
    sequence,
    comment,
...
console.log(JSON.stringify({ inputPath, ...parsed }, null, 2));
